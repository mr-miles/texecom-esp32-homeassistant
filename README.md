...
Once flashed, point Wintex (or any TCP client) at `texecom-bridge-<device_id>.local:10001` (the `device_id` value comes from your YAML). Only one client is allowed at a time; a second connection is rejected cleanly while the first session is preserved.
